...
static uint16_t temp_l,temp_m,temp_h;
    ...
    uint16_t x_ratio = 0;
    uint16_t y_ratio = 0;
	int32_t ov5642_TouchAF_x = 0;
	int32_t ov5642_TouchAF_y = 0;
 ...
 * Auto Focus Trigger
 */
 //modified the interface of touch AF ,ZTE_YGL_CAM_20110302
...
if(ov5642_TouchAF_x == 0 || ov5642_TouchAF_y == 0){
    ...
    */
    ...
    rc = ov5642_i2c_write(ov5642_client->addr, 0x5084, (int32_t)ov5642_TouchAF_x, WORD_LEN);
    if (rc < 0)
    {
	   goto fail;
    }
    rc = ov5642_i2c_write(ov5642_client->addr, 0x5085, (int32_t)ov5642_TouchAF_y, WORD_LEN);
    if (rc < 0)
    {
	   goto fail;
    }
...
static int32_t ov5642_set_aec_rio(aec_rio_cfg position)
{
	...
	x_ratio = position.preview_width/OV5642_AF_WINDOW_FULL_WIDTH;
    y_ratio = position.preview_height/OV5642_AF_WINDOW_FULL_HEIGHT;
	ov5642_TouchAF_x = position.x / x_ratio;
	ov5642_TouchAF_y = position.y / y_ratio;
...
/*
 * ZTE_CAM_LJ_20110427
 * restore af status before sensor enter  standby mode,
 * else it will cause high current
 */
