...
/*===========================================================================
  ...
  ----------   ---   -----------------------------------------------------------
  2011-01-30  zj     add sysfs interface for getting ri state	                       	ZTE_GSM_MMS
...
struct modemctl_ops {
	...
	int (*get_workingstate)(void);
       int (*get_ristate)(void); // zhoujing ZTE_GSM_MMS  20110125
	int (*open_modem)(void);	
...
struct modemctl_sysfs_info {
	...
	unsigned int cmd;
    	...
    	unsigned int ri_state;     // zhoujing ZTE_GSM_MMS  20110125, the ri state
};
