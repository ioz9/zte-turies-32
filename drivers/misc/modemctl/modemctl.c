...
static ssize_t modemctl_show_ri_state(struct device_driver *driver, char *buf);  // zhoujing ZTE_GSM_MMS 20110125
...
static DRIVER_ATTR(ri_state, S_IRUGO, modemctl_show_ri_state, NULL); // read-only Zhoujing ZTE_GSM_MMS  20110125
...
static void modemclt_init_sysfs_info(void)
{
    ...
    p_mdi->sysfs_info->ri_state = 0; //zhoujing ZTE_GSM_MMS 20110125
}
...
static int modemclt_create_driver_sysfs(struct platform_driver *drv)
{
    ...
    error |= driver_create_file(&drv->driver, &driver_attr_ri_state);   // zhoujing ZTE_GSM_MMS 20110125
    MODEMCTL_INFO("--");
    return error;
}
...
static void modemclt_remove_driver_sysfs(struct platform_driver *drv)
{
    ...
    driver_remove_file(&drv->driver, &driver_attr_ri_state);   // zhoujing ZTE_GSM_MMS 20110125 
    MODEMCTL_INFO("--");
}
...
**********************************************************************/
static ssize_t modemctl_show_ri_state(struct device_driver *driver, char *buf)
{
    unsigned int ristate = 0;
    ristate = p_mdi->ops->get_ristate(); // RI state
...
static irqreturn_t modemctl_hostwake_isr(int irq, void *dev_id)
{
    ...
    printk(KERN_INFO "modemctl_hostwake_isr : ri signal : %d \n", p_mdi->sysfs_info->ri_state); // zhoujing ZTE_GSM_MMS 20110125 
