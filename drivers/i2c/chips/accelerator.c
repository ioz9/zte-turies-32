...
extern int get_zte_board_id_type(void); 
static int blade_board_id = 0;
#endif
...
#ifdef CONFIG_MOONCAKE_USED_SMALL_WRAPPER_SENSORS
	buffer[0] = (s16) le16_to_cpu(buf[1])*(-1)*4;
	buffer[1] = (s16) le16_to_cpu(buf[0])*4;
	buffer[2] = (s16) le16_to_cpu(buf[2])*(-1)*4;
#endif

#if defined(CONFIG_MACH_MINIRACER)
	buffer[0] = (s16) le16_to_cpu(buf[1])*(-1)*4;
	buffer[1] = (s16) le16_to_cpu(buf[0])*4;
	buffer[2] = (s16) le16_to_cpu(buf[2])*(-1)*4;
#endif
				       ...
				       const struct i2c_device_id *id)
{
	...
	//ZTE_FYA_ADD_20101119 end	--------------------	
		pr_info(LIS302_TAG"Do not to cheackout chipid for lis33de!\n");
#else
		ret = lis302dl_i2c_rx(buf, 1);
		if(ret < 0) {
			...
			if(LIS302DL_WHO_AM_I_MAGIC == buf[0]) {
				pr_err(LIS302_TAG "our device used lis302dl && lis35de!\n");
				//goto probe_failed;
			}else{
				pr_info(LIS302_TAG"our device used lis33de\n");
			}
		
			device_flag = 0;
			pr_info(LIS302_TAG "use ST gsenosr chip!\n");
		}			
#endif
	}
	if(1 == device_flag){
