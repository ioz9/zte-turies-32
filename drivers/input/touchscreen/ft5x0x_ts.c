...
2011-05-11  zfj     cancel rotate x and y                               ZTE_TS_ZFJ_20110511
...
2011-02-15	zfj		modify TSEN and TSINT GPIO for P725A 				ZTE_TS_ZFJ_20110215
...
#define GPIO_TOUCH_EN_OUT 89  	//ZTE_TS_ZFJ_20110215
#else
#define GPIO_TOUCH_EN_OUT  31
#endif
...
static const char ts_keys_size[] = "0x01:102:35:498:70:36:0x01:139:160:498:70:36:0x01:158:285:498:70:36";
...
static void Fts_ts_work_func(struct work_struct *work)
{
	...
	if (ret < 0){
   		pr_err("Fts_ts_work_func: Fts_i2c_write failed, go to poweroff.\n");
	...
	{
		//pr_info("Fts Touch Event: P1=%x,P2=%x,Points=%x\n",buf[3] ,buf[9],buf[2] );
...
Fts_resume_start:	
	...
	{//I2C error read firmware ID
		pr_info("Fts FW ID read Error: retry=%x\n",retry);
		if ( ++retry < 3 )
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret)
	{	
		pr_info("gpio 31 request is error!\n");
	...
	{
		pr_err("Fts_ts_probe: need I2C_FUNC_I2C\n");
		ret = -ENODEV;
		goto err_check_functionality_failed;
	}
	...
	if (ts->input_dev == NULL) {
		ret = -ENOMEM;
		pr_err("Fts_ts_probe: Failed to allocate input device\n");
...
/*
#if defined(CONFIG_MACH_TURIES)
	p725a_max_y = SCREEN_MAX_Y;
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, SCREEN_MAX_Y, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, SCREEN_MAX_X, 0, 0);
#else	
*/
/*ZTE_TS_ZFJ_20110511 end*/
...
/*ZTE_TS_ZFJ_20110511 end*/
	...
	if (ret)
	{
		pr_err("Fts_ts_probe: Unable to register %s input device\n", ts->input_dev->name);
