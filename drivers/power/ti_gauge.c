...
int bat_read_gauge(int reg,unsigned int* readvalue)
{
        ...
        if(rc==-1)//电池存在才允许读
        {
            ...
            if(rc==0)
            {
                if(REGADDR_VOLT==reg)
                {
                    ...
                    {
                        printk( "%s,voltage=%u out of range\n", __func__,*readvalue);
                ...
                if(REGADDR_SOC==reg)  // 4v高电压出现0%的情况，应该是开机测开路电压时有电流造成的，如果没有保障2s的等待时间，在这里优化一下
                {//如果容量为0，则看看电压是否高于3.4v，高于则使用modem侧的容量
                    ...
                    else if((*readvalue==100))//容量为100时如果还有充电电流，则容量限制在99%
                    {
                        rc=gauge_standard_read(REGADDR_AI,&ai);
                        if(rc==0)
                        {
                            if(ai>0)
                            {
                        ...
                        else//电流读取失败，不太可能发生，return -1
                        {
                            printk( "%s:%d,read current failed!\n", __func__,__LINE__);
                        }
                    }
                    else if((*readvalue==0))
                {//如果容量为0，则看看电压是否高于3.4v，高于则使用modem侧的容量
                    ...
                    if(rc==0)
                    {
                        ...
                        else //电压读取失败，不太可能发生,return -1
                        {
                            printk( "%s:%d,read voltage failed!\n", __func__,__LINE__);
                        }
                    }
                    else
                    {
    ...
    {
        printk( "%s, bat can't read gauge!enable=%u,updating=%u,gauge_initialized=%u\n", __func__,gauge_enable,testbit(0),gauge_initialized);
    }
    if(rc)
    {
