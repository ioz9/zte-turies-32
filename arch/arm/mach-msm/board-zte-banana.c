...
/*ZTE_TRACE_BALL_001,@2010-04-19,BEGIN*/
#ifdef CONFIG_JOGBALL_KEY
#include <mach/jogball_key.h>
#endif
/*ZTE_TRACE_BALL_001,@2010-04-19,END*/
...
static uint32_t lcdc_gpio_table[] = {
	...
	GPIO_CFG(GPIO_LCD_PANEL_ID, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),};
#else
static int gpio_array_num[] = {
...
//ZTE_MODEMCTL_WXW_DEVICE_001, begin, add the modemctl device resource
...
//ZTE_MODEMCTL_WXW_DEVICE_001, end, add the modemctl device resource


//ZTE_MODEMCTL_WXW_GPIOCONFIG_001, begin, add the GPIO config of modemctl
...
//make the rts and cts as gpio pin
//static unsigned modemctl_config_rts = GPIO_CFG(MSM_GPIO_UARTDM2_RTS, 0, GPIO_OUTPUT,  GPIO_NO_PULL, GPIO_2MA);	/* Rx */
//static unsigned modemctl_config_cts = GPIO_CFG(MSM_GPIO_UARTDM2_CTS, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA);	/* Tx */

static int modemctl_init_uartdm2_gpio(void)
{
	int rc;
      printk(KERN_ERR "@@@@@@ enter %s: config the GPIO UART DM2 ++\n",__func__);
      //printk(KERN_ERR "@@@ don't config the any GPIO UART DM2 \n");
...
/*
    	rc = gpio_tlmm_config(modemctl_config_rts,GPIO_ENABLE);
	if (rc) {
		printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n",__func__, MSM_GPIO_UARTDM2_RTS, rc);
		return -EIO;
	}
	rc = gpio_request(MSM_GPIO_UARTDM2_RTS, "modemctl");
	if(rc)
	{
		printk(KERN_ERR "gpio_request: %d failed!\n", MSM_GPIO_UARTDM2_RTS);
	}

    	rc = gpio_tlmm_config(modemctl_config_cts,GPIO_ENABLE);
	if (rc) {
		printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n",__func__, MSM_GPIO_UARTDM2_CTS, rc);
		return -EIO;
	}
	rc = gpio_request(MSM_GPIO_UARTDM2_CTS, "modemctl");
	if(rc)
	{
		printk(KERN_ERR "gpio_request: %d failed!\n", MSM_GPIO_UARTDM2_CTS);
	}
...
//#endif
//ZTE_MODEMCTL_WXW_GPIOCONFIG_001, end, add the GPIO config of modemctl
 ...
 */
 /*
  * removed the iovdd setting to ARM9, ZTE_YGL_CAMERA_20110302
  */
...
MACHINE_START(BANANA, "banana ZTE handset")
#ifdef CONFIG_MSM_DEBUG_UART
	.phys_io        = MSM_DEBUG_UART_PHYS,
	.io_pg_offst    = ((MSM_DEBUG_UART_BASE) >> 18) & 0xfffc,
#endif
