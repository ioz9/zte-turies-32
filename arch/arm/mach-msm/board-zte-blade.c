...
#ifdef CONFIG_ZTE_PLATFORM
#define BOARD_LOW_COST 1
#define BOARD_NORMAL_COST 0

#ifdef CONFIG_MACH_BLADE
	#define GPIO_BOARD_ID 88
#else
	#define GPIO_BOARD_ID 0xFF	
#endif
...
static int32_t blade_lcst_camera_pwrup_sequence(struct vreg *vreg_cam_dvdd, struct vreg *vreg_cam_avdd, struct vreg *vreg_cam_iovdd)
{
    int32_t rc_cam_dvdd, rc_cam_avdd, rc_cam_iovdd;
...
int32_t msm_camera_power_backend(enum msm_camera_pwr_mode_t pwr_mode)
{
    ...
    switch (pwr_mode)
    {
        case MSM_CAMERA_PWRUP_MODE:
        {
            if(get_zte_board_id_type() == BOARD_LOW_COST)//for low cost board,ZTE_CAM_GUOYANLING2011422
            {
                ret = blade_lcst_camera_pwrup_sequence(vreg_cam_dvdd, vreg_cam_avdd, vreg_cam_iovdd);
                if(ret != 0)
                    return -EIO; 
            }
            else//for normal board before,ZTE_CAM_GUOYANLING2011422
            {
        ...
        case MSM_CAMERA_STANDBY_MODE:
        {
            if(get_zte_board_id_type() != BOARD_LOW_COST)
            {
                rc_cam_avdd  = vreg_disable(vreg_cam_avdd);
                if (rc_cam_avdd)
                {
...
static struct i2c_board_info aux2_i2c_devices_lowcost[] = {
#ifndef CONFIG_MSENSORS_FROM_AUXI2C_TO_I2C
	{
		I2C_BOARD_INFO("akm8962",0x0c),
	},
...
static int read_gpio_board_id(int gpio_board_id)
{
	...
	if(gpio_board_id >=0xff )
		return 0;
		
	err = gpio_request(gpio_board_id,"board_id");
	if(err) {
		printk(KERN_ERR "gpio_request: %d failed!\n",gpio_board_id);
		return err;
   }
   
   err = gpio_direction_input(gpio_board_id);
   	if(err) {
		printk(KERN_ERR "gpio_direction_input: ret = %d failed!\n",err);
		return err;
   }

   for( i = 0; i < NUM_READ; i++){
   	if (gpio_get_value(gpio_board_id) > 0) count++;
   	printk(KERN_ERR "gpio_get_value: ret = %d !\n",gpio_get_value(gpio_board_id));
   }
   
	 if (count >= (NUM_READ + 1)/2) 
	  ret = BOARD_LOW_COST;
	 else
	 	ret = BOARD_NORMAL_COST;
	 	
	  printk(KERN_INFO "gpio_board_id: %d,ret = %d !\n",gpio_board_id,ret);
	 	gpio_free(gpio_board_id);
	 	return 	ret;	 	
}


static void set_zte_board_id_type(int flag)
{
	extern void sync_sysfs_board_id(int flag);
	g_zte_board_id_type = flag > 0?BOARD_LOW_COST:BOARD_NORMAL_COST;
...
#ifdef CONFIG_ZTE_PLATFORM
set_zte_board_id_type(read_gpio_board_id(GPIO_BOARD_ID));
#endif


//ZTE_CAM_GUOYANLING20110422
    if(get_zte_board_id_type() == BOARD_LOW_COST)
...
/*Description for hardware different in lowcost blade*/	
	if(get_zte_board_id_type()== 0)
		i2c_register_board_info(2, aux2_i2c_devices, ARRAY_SIZE(aux2_i2c_devices));
	else if(get_zte_board_id_type()== 1)
		i2c_register_board_info(2, aux2_i2c_devices_lowcost, ARRAY_SIZE(aux2_i2c_devices_lowcost));
	else
	i2c_register_board_info(2, aux2_i2c_devices, ARRAY_SIZE(aux2_i2c_devices));
/* End for hareware different in lowcost blade*/
